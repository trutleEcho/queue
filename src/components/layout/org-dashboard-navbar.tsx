...
export default function OrgDashboardNavbar({ orgId }: { orgId: string }) {
    return (
        <div className="top-0 left-0 z-50 bg-gradient-to-b from-background via-background/80 to-background/10 rounded-b-2xl">
            <header className="relative z-50 p-4 lg:p-6 flex justify-between items-center">
                ...
                <div className="flex items-center space-x-2 md:space-x-6">
                    ...
                    <Link href={`/org/${orgId}`} className="text-muted-foreground hover:text-foreground transition-colors text-sm">Locations</Link>
                    <ThemeToggle />
                </div>
            </header>
        </div>
